...
(async () => {
  try {
    ...
    let frontend = process.env.FRONTEND_URL || process.env.NEXT_PUBLIC_FRONTEND_URL || 'http://localhost:3000';
    let headless = true;
    ...
    argv.forEach((a) => {
      ...
      if (a === '--help' || a === '-h') {
        console.log('Usage: node e2e/pw-e2e.js [--url=http://localhost:3000] [--headless=true|false]');
        process.exit(0);
      }
    });
    ...
    console.log('Launching browser (headless=' + headless + ') and visiting', signupUrl);
    ...
    try { await page.waitForSelector('input[name="name"]', { timeout: 3000 }); } catch (e) {}
    await page.type('input[name="name"]', testName, { delay: 50 }).catch(() => {});
    await page.type('input[name="email"]', testEmail, { delay: 50 }).catch(() => {});
    await page.type('input[name="password"]', testPassword, { delay: 50 }).catch(() => {});
    await page.type('input[name="confirmPassword"]', testPassword, { delay: 50 }).catch(() => {});
    ...
    if (tokenCookie) console.log('Token cookie details:', { name: tokenCookie.name, httpOnly: tokenCookie.httpOnly, secure: tokenCookie.secure, expires: tokenCookie.expires });
    ...
    try {
      ...
      const out = execSync('node backend/scripts/listUsers.js', { encoding: 'utf8' });
      console.log(out);
    } catch (err) {
